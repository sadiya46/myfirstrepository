...
locals {
  cloud_watch_group_name = "${var.app_name}-${var.environment}-log"
  cluster_name           = "she-${var.environment}-eks"
  dns_prefix             = "${var.app_name}-${var.environment}-k8s"
}

# create a aws_eks_cluster #
resource "aws_eks_cluster" "she-dev-eks" {
...
resource "aws_cloudwatch_log_group" "she-eks-cwg" {
  ...
  #name                   = "${local.cloud_watch_group_name}-${random_id.eks[count.index].dec}"
...
resource "aws_eks_node_group" "she-dev-group" {
  cluster_name    = aws_eks_cluster.she-dev-eks.name
