...
resource "aws_eks_cluster" "she-eks" {
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.she_ssh.id]
    subnet_ids         = [aws_subnet.dev-subpub01.id, aws_subnet.dev-subpub02.id]
...
resource "aws_eks_node_group" "she-dev-group" {
  ...
  subnet_ids      = aws_subnet.dev-subpub01[*].id
  // subnet_ids      = aws_subnet.dev-subpub02[*].id
...
# Creating IAM Roles and policies on a public subnet for NodeGroup #
